let fields = [], availableHeaders = [], allRows = [], fontBytes = null;
const sheetInput = document.getElementById('sheetInput'),
      pdfInput = document.getElementById('pdfInput'),
      filenameColumn = document.getElementById('filenameColumn'),
      fontInput = document.getElementById('fontInput'),
      filterColumnInput = document.getElementById('filterColumn'),
      filterValueInput = document.getElementById('filterValue'),
...
sheetInput.addEventListener('change', async () => {
  ...
  filterColumnInput.innerHTML = '<option value="">None</option>' + 
    availableHeaders.map(h=>`<option>${h}</option>`).join('');
  allRows = raw.slice(1).map(r=>{
    const o={}; availableHeaders.forEach((h,i)=>o[h]=r[i]||''); return o;
  });
  filterValueInput.innerHTML = '';
  status.textContent = `Loaded columns: ${availableHeaders.join(', ')}`;
});

filterColumnInput.addEventListener('change', () => {
  const col = filterColumnInput.value;
  filterValueInput.innerHTML = '';
  if (!col) return;
  const set = new Set();
  allRows.forEach(r=>{ const v = String(r[col]||'').trim(); if(v) set.add(v); });
...
pdfInput.addEventListener('change', async () => {
  ...
  const page = await pdf.getPage(1), vp = page.getViewport({scale:1.5});
  ...
  templateArea.style.position = 'relative';
  // re-append wrappers so they are visible above
  fields.forEach(f=>templateArea.appendChild(f.wrapper));
});
...
addFieldBtn.onclick = () => {
  ...
  input.innerText = '{{ColumnName}}'; input.style.direction = 'rtl';
  const select = document.createElement('select');
  select.innerHTML = `<option disabled selected>Bind column</option>` + 
    availableHeaders.map(h=>`<option>${h}</option>`).join('');
  ...
  deleteBtn.textContent='❌'; deleteBtn.style.fontSize='12px'; deleteBtn.style.padding='2px 6px';
  deleteBtn.onclick = ()=>{
    templateArea.removeChild(wrapper);
    fields = fields.filter(f=>f.wrapper!==wrapper);
  };
  const posLabel = document.createElement('div');
  posLabel.className = 'pos-label'; posLabel.innerText = 'X:20, Y:20';
  ...
  fields.push({wrapper,input,select,fontSelect,sizeInput,widthInput,colorInput,alignSelect,posLabel});
...
function makeDraggable(el,label){
  ...
  el.onmousedown = e => {
    if (e.target.isContentEditable || ['INPUT','SELECT','BUTTON'].includes(e.target.tagName)) return;
...
function shouldGenerate(row, filterCol, filterVals) {
  if (!filterCol || !filterVals.length) return true;
  const cv = String(row[filterCol]||'').trim().toLowerCase();
  return filterVals.some(v=>v.trim().toLowerCase()===cv);
}

generateBtn.onclick = async () => {
  status.textContent = 'Processing...'; generateBtn.disabled = true;
  try {
    ...
    const filterCol = filterColumnInput.value;
    const filterVals = Array.from(filterValueInput.selectedOptions).map(o => o.value.trim());
    const zip = new JSZip(); let count=0;
    for (const row of rows) {
      if (!shouldGenerate(row, filterCol, filterVals)) continue;
      ...
      const page = pdfDoc.getPage(0); const {width:pw,height:ph} = page.getSize();
      ...
      for (const f of fields) {
        ...
        const font = customFont || await pdfDoc.embedFont(PDFLib.StandardFonts[f.fontSelect.value]||PDFLib.StandardFonts.Helvetica);
        ...
        const lines = splitTextIntoLines(txt,font,sz,widthBox);
        let offsetY = 0;
        for (const line of lines) {
          ...
          page.drawText(line, { x: drawX, y: yPDF - offsetY, size: sz, font, color: PDFLib.rgb(color.r/255,color.g/255,color.b/255), maxWidth: widthBox });
          offsetY += sz * 1.2;
        }
      }
      const pdfBytes = await pdfDoc.save();
      const safe = (row[fnameKey]||'output').toString().replace(/[\\/:"*?<>|]+/g,'_') || `file${count}`;
      zip.file(`${safe}.pdf`, pdfBytes);
      count++;
      status.textContent = `Generated ${count}…`;
    }
    if (count === 0) status.textContent = 'No matching rows.';
    else {
      const blob = await zip.generateAsync({ type: 'blob' });
      saveAs(blob, 'merged.zip');
      status.textContent = `Generated ${count} PDFs successfully!`;
    }
  } catch(err) {
    console.error(err);
